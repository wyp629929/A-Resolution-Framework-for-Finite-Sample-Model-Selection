...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_synthetic_validation.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_simulation_power_curve.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_bootstrap_selection.png}
...
\begin{table*}[t]
\centering
\caption{Consolidated view of experimental settings. Ranges and thresholds in percentage points. $\Delta_{\min}$ is the independent MDE$_{50}$ (Eq.~\ref{eq:delta_min}) at each setting's $\bar{p}$. The Pairwise column applies the uncorrected MDE$_{50}$ screening criterion; the Multiplicity column applies the all-pairwise ($m=45$) Bonferroni correction.}
\label{tab:all_settings}
\centering
\small
\setlength{\tabcolsep}{2.5pt}
\begin{tabular}{lcp{24mm}lrrrrl}\toprule
\textbf{\S} & $\bm{N}$ & \textbf{Setting} & \textbf{Prot.} & \textbf{Range} & $\bm{\Delta_{\min}^{\text{(ind)}}}$ & \textbf{Pair.} & \textbf{Mult.} \\\midrule
\ref{sec:full_budget} & 1319 & Qwen2.5-7B LoRA GSM8K full & train. & 3.0 & 3.8 & --- & --- \\
\ref{sec:budget_degradation} & 200 & Qwen2.5-7B GSM8K sub. & few-sh. & 8.0 & 9.8 & Not resolv. & --- \\
\ref{sec:budget_degradation} & 200 & Qwen2.5-7B seed 42 (128) & train. & 5.5 & 9.8 & Not resolv. & --- \\
\ref{sec:seeds} & 200 & Qwen2.5-7B seed 42 (256) & train. & 12.5 & 9.7 & Resolvable & Not resolv. \\
\ref{sec:seeds} & 200 & Qwen2.5-7B seed 43 (256) & train. & 14.0 & 9.1 & Resolvable & Not resolv. \\
\ref{sec:seeds} & 200 & Qwen2.5-7B seed 44 (256) & train. & 13.5 & 9.1 & Resolvable & Not resolv. \\
\ref{sec:small_scale} & 200 & Qwen2.5-1.5B LoRA & zero-sh. & 3.5 & 3.4 & \emph{Invalid} & --- \\
\ref{sec:full_ft} & 200 & Qwen2.5-1.5B full ft. & train. & 6.0 & 9.4 & Not resolv. & --- \\
\ref{sec:mistral} & 200 & Mistral-7B LoRA & train. & 11.0 & 8.8 & \textbf{Resolvable} & Not resolv. \\
\ref{sec:mmlu} & $\approx$250 & Qwen2.5-7B MMLU 5 subd. & --- & 6.4 & 8.8 & Not resolv. & --- \\
\ref{sec:mnist} & 5000 & MNIST random forest & --- & --- & 1.4 & See Table~\ref{tab:mnist} & --- \\
\bottomrule
\end{tabular}
\end{table*}
