...
\abstract{A single decoding parameter---the maximum generation length---can reverse whether a set of model checkpoints is classified as distinguishable or indistinguishable, and can conceal a statistically significant overfitting trend ($\rho=-0.77$, $p=0.009$) behind a facade of flat performance. We document this through controlled LoRA fine-tuning experiments (Qwen2.5-7B, Mistral-7B) in which increasing the generation length from $128$ to $256$~tokens shifts observed accuracy ranges by $2.3\times$, flips resolvability verdicts, and reveals that the earliest checkpoint is systematically best---a pattern invisible under the shorter limit. The mechanism is truncated chain-of-thought reasoning: a short generation limit compresses between-checkpoint variance, manufacturing false negatives for the $\Delta_{\min}$ diagnostic (a resolution threshold derived from classical power analysis). We survey 50 recent fine-tuning papers and find that none report the generation length parameter that we show can determine the verdict. The protocol sensitivity of evaluation resolution means that no model comparison claim is interpretable unless the evaluation pipeline is fully specified. A catalogue of nine diagnostic pitfalls encountered during our own research process, a practitioner quick-reference table for $\Delta_{\min}$, and a continuous-metric extension via the Delta method accompany the analysis.}
...
Two additional random seeds (43,~44) were trained. Under the $128$-token evaluation used throughout Section~\ref{sec:budget_degradation}, seed~43 has range $5.0$~pp and seed~44 has range $3.0$~pp, both below $\Delta_{\min}=9.8$~pp. However, as Section~\ref{sec:adaptive} documents, these ranges are compressed by token truncation. At $256$~tokens, all three seeds become resolvable: seed~43 has range $14.0$~pp and seed~44 has range $13.5$~pp, both exceeding $\Delta_{\min}$. Critically, the $256$-token data reveal a pattern that the $128$-token evaluation entirely masked: seeds~43 and~44 show significant negative trends (Spearman $\rho=-0.77$, $p=0.009$ and $\rho=-0.63$, $p=0.050$ respectively), with the earliest checkpoint (step~50) outperforming the last by $12.5$~pp and $10.5$~pp (best-vs-worst $z=2.82$, $p=0.005$ and $z=2.33$, $p=0.020$). This is evidence of overfitting after the initial learning phase, a pattern that is invisible under a generation limit that truncates the model's output. Seed~42 does not share this trend ($\rho=+0.29$, $p=0.43$), confirming that protocol sensitivity and overfitting susceptibility are trajectory-specific as well as model-specific.
...
\begin{tabular}{llcrrrrl}\toprule
\textbf{\S} & \textbf{Setting} & \textbf{Protocol} & $\bm{N}$ & \textbf{Range} & $\bm{\Delta_{\min}^{\text{(indep)}}}$ & $\bm{\Delta_{\min}^{\text{(paired)}}}$ & \textbf{Verdict} \\\midrule
\ref{sec:full_budget} & Qwen2.5-7B LoRA, GSM8K full & training & 1319 & 3.0 & 3.8 & 2.9--3.5 & Not resolvable \\
...
\ref{sec:seeds} & Qwen2.5-7B LoRA, seed 42 (256~tok) & training & 200 & 12.5 & 9.7 & 8.7--9.3 & Resolvable \\
...
\ref{sec:mistral} & Mistral-7B LoRA & training & 200 & 11.0 & 8.8 & 7.2--7.9 & \textbf{Resolvable} \\
...
The $256$-token data also reveal a finding that the $128$-token evaluation entirely concealed: seeds~43 and~44 show statistically significant negative trends under the longer generation limit (Spearman $\rho=-0.77$, $p=0.009$ and $\rho=-0.63$, $p=0.050$), with the earliest checkpoint (step~50) outperforming the final checkpoint by $12.5$~pp and $10.5$~pp respectively. This is evidence of overfitting after an initial learning phase---a pattern invisible when the model's output is truncated. Seed~42 does not share this trend ($\rho=+0.29$, $p=0.43$), confirming that protocol sensitivity and overfitting susceptibility are trajectory-specific. All $256$-token checkpoints are reported in Appendix~\ref{tab:seed_rep}.
