...
\title[Protocol-Dependent Model Selection]{Protocol-Dependent Resolution in Finite-Sample Model Selection}

\author{Yaoping Wang}
\affil{Department of Statistics, Rutgers University, Piscataway, NJ, USA}
\email{yw1580@scarletmail.rutgers.edu}

\abstract{The maximum number of tokens a model is allowed to generate can reverse whether a set of checkpoints looks distinguishable or indistinguishable, and can hide a strong negative trend ($\rho=-0.77$) behind apparently flat performance. We document this through controlled LoRA experiments (Qwen2.5-7B, Mistral-7B) where moving from $128$ to $256$~tokens shifts accuracy ranges by $2.3\times$, flips pairwise resolution verdicts, and reveals a negative trend in two of three seeds that is invisible under the shorter limit. The mechanism is truncated chain-of-thought reasoning: a short generation limit compresses between-checkpoint variance and produces false negatives for $\Delta_{\min}$, the minimum detectable effect at $50\%$ power (MDE$_{50}$) for a two-proportion $z$-test. This threshold is a conservative lower bound on evaluation resolution, not a sufficient-power standard. A survey of 50 recent fine-tuning papers finds that none report the generation length parameter that our experiments show can determine the verdict. If the evaluation pipeline is incompletely specified, no model comparison claim is interpretable. A catalogue of nine diagnostic pitfalls, a practitioner quick-reference table for $\Delta_{\min}$, and a continuous-metric extension via the Delta method accompany the analysis.}
...
Model selection across data science and machine learning follows a default rule: evaluate each candidate on a held-out validation set and pick the one with the best metric. The rule assumes that a model scoring $55\%$ is genuinely better than one scoring $52\%$ on the same test set. This assumption is known to be fragile~\citep{kaur2025instruct}, but the source of the fragility is debated. Is ranking failure a rare event caused by distribution shift or overfitting? Or is it a systematic limitation of finite-budget evaluation?

We argue that it is the latter. Model ranking is constrained by the evaluation budget, the same way any statistical estimator is constrained by sample size. Existing comparison tools, including paired $t$-tests~\citep{dietterich1998approximate}, significance guidelines~\citep{dror2018hitchhiker}, and post-selection inference~\citep{zrnic2024flexible,zhang2024winners}, are post-hoc: they assess results after evaluation is complete. What is missing is a \emph{pre-experimental diagnostic} that determines, before resources are spent, whether the evaluation budget can support statistically distinguishable comparisons.

This paper proposes such a diagnostic. In applying it, we find something arguably more consequential than the diagnostic itself. The diagnostic, $\Delta_{\min}$, is a resolution threshold derived from the two-proportion $z$-test~\citep{cohen1988statistical} and is a textbook formula. But applying it reveals that the verdict it produces can be reversed by a single decoding parameter, the maximum generation length, that almost nobody reports. Experiments across multiple seeds and two model families show that a short generation limit compresses between-checkpoint variance. It creates the appearance of indistinguishability, hides genuine overfitting trends, and inverts which checkpoint looks best. We document nine diagnostic pitfalls encountered during the research process and show that the problem of underpowered evaluation is compounded by a problem of unaccountable evaluation. If the pipeline that produced the numbers is incompletely specified, the resolution of those numbers cannot be independently assessed.

\subsection*{Contributions}

\begin{enumerate}
    \item \textbf{An empirical demonstration of protocol-dependent resolution.} A single decoding parameter---maximum generation length---shifts observed accuracy ranges by $2.3\times$, reverses pairwise resolution verdicts across three training seeds, and conceals evidence of overfitting. The $\Delta_{\min}$ diagnostic (MDE$_{50}$ threshold from classical power analysis) is the tool that reveals this sensitivity. The pattern is replicated across two model families (Qwen2.5-7B, Mistral-7B), multiple seeds, scales and training paradigms, and on classical ML model selection (random forests on MNIST).
    ...
    \item \textbf{A literature survey} documenting that none of 50 recent fine-tuning papers report the maximum generation length that determines whether the MDE$_{50}$ screening threshold is exceeded.
\end{enumerate}
...
\label{eq:delta_min}
\end{equation}
Equation~(\ref{eq:delta_min}) is the standard minimum detectable effect for a two-proportion $z$-test~\citep{cohen1988statistical}, restated as a pre-experimental resolution diagnostic. The independent version assumes no shared variance, so it overstates the true threshold when checkpoints agree on many examples. Throughout this paper, all reported $\Delta_{\min}$ values use Eq.~\eqref{eq:delta_min} (the conservative bound). Any practitioner can compute this from $N$ and $\bar{p}$ alone. Where per-sample predictions are available, we also report the tighter paired thresholds. Equation~(\ref{eq:delta_min}) gives approximately $50\%$ power; we therefore refer to it as MDE$_{50}$. An $80\%$ power target would require $\Delta_{\min}^{(80\%)}=14.0$~pp at $N=200$, $\bar{p}=0.5$. Throughout this paper, ``resolvable'' means that the observed gap exceeds the $50\%$-power threshold (MDE$_{50}$)---a screening criterion, not a fully powered confirmatory test. The $80\%$-power threshold is $1.43\times$ larger.
...
\textbf{Scaling laws for ranking resolution.} The $\Delta_{\min}$ formula (Eq.~\ref{eq:delta_min}) captures two fundamental scaling relationships that govern when checkpoint ranking is reliable.

\noindent\textbf{Budget scaling (Implication 1).} $N \propto 1/\delta^2$: halving the minimum detectable gap requires quadrupling the evaluation budget. This follows directly from the $z$-test statistic: the standard error of a proportion scales as $\sqrt{\bar{p}(1-\bar{p})/N}$, so the detectable effect scales as $1/\sqrt{N}$.

\noindent\textbf{Multiple comparison scaling (Implication 2).} When selecting the best from $m$ candidates, the relevant threshold tightens to $\Delta_{\min}^{(m)} \approx z_{\alpha/2m}\sqrt{2\bar{p}(1-\bar{p})/N}$, which scales as $\sqrt{\log m}$ under the Bonferroni correction (Proof in Appendix~\ref{app:proof}). For $m=10$ at $N=200$, $\Delta_{\min}^{(10)}\approx 14.0$~pp (Bonferroni-corrected $\alpha=0.005$).\footnote{The numerical coincidence that the $80\%$-power pairwise threshold also equals $14.0$~pp at $N=200$ is incidental; the two values arise from different corrections (one for power, one for multiplicity).} Because checkpoint accuracies are positively correlated (adjacent steps differ by small amounts), Bonferroni is conservative, and we report it as a strict upper bound. For the $45$ pairwise comparisons among $10$ checkpoints, the Bonferroni-corrected threshold is $16.2$~pp ($z_{0.05/90}=3.26$). None of the three $256$-token seeds exceed this threshold---under any multiple-comparison correction, $N=200$ is insufficient to support a pairwise resolvable verdict after accounting for the number of comparisons made. This does not conflict with the screening role of the uncorrected MDE$_{50}$ thresholds, which are designed to indicate when a comparison merits closer inspection rather than to certify a finding.
...
We first evaluate the full GSM8K test set ($N=1319$, training-format prompt, $128$-token generation) for the seed~42 trajectory. At this budget, $\Delta_{\min}=3.8$~pp, yet the observed checkpoint accuracy range is only $3.0$~pp. (This evaluation uses $128$-token generation; the range may be compressed by truncation, as documented in \S\ref{sec:protocol_sensitivity}.) The ranking is not statistically resolvable even at the full test set size: the observed differences between checkpoints are smaller than the minimum gap detectable at this evaluation budget.
...
\textbf{Trend test (few-shot protocol).} Spearman $\rho_{\text{trend}}=-0.091$ ($p=0.803$). No monotonic trend.

\textbf{Adjacent comparison test (few-shot protocol).} Two-proportion $z$-tests for adjacent pairs show no significant differences under any standard correction: the smallest $p$ is $0.161$ (Bonferroni threshold $\alpha' = 0.05/9 \approx 0.0056$). (Full table in Appendix~\ref{app:adjacent}.) This adjacent-pair correction addresses a different comparison family from the all-pairwise correction (Implication~2); the two answer whether any step-to-step change is significant versus whether any two checkpoints are distinguishable.

\textbf{Range test (few-shot protocol).} Best (step~150, $55.5\%$) vs.\ worst (step~50, $47.5\%$): $z=1.60$, $p=0.109$.

\textbf{Paired comparison (training-format protocol).} All nine adjacent comparisons under the training-format evaluation are non-significant (smallest $p=0.265$). Best-vs-worst: $p=0.091$.

\textbf{Bootstrap ranking stability (training-format protocol).} We performed 10,000 nonparametric bootstrap resamples of the 200 evaluation examples (preserving paired structure), using the training-format evaluation. The most frequently selected checkpoint (step~100, $52.0\%$) is chosen in only $44.0\%$ of replicates (Figure~\ref{fig:bootstrap}; normalized entropy $0.717$). The identity of the empirically best model depends largely on which evaluation examples are observed.
...
Table~\ref{tab:all_settings} collects every experimental setting reported in \S\ref{sec:primary} and \S\ref{sec:robustness} with its evaluation protocol, budget, observed range, threshold and verdict. The same Qwen2.5-7B checkpoints appear five times at $N=200$ under different protocols: the few-shot evaluation (range $8.0$~pp), the $128$-token training-format evaluation ($5.5$~pp), and three $256$-token training-format evaluations showing all three seeds resolvable under the MDE$_{50}$ screening criterion ($12.5$, $14.0$, $13.5$~pp). The $2.5$--$9.0$~pp discrepancy between protocols---driven by a single decoding parameter (\S\ref{sec:protocol_sensitivity})---is itself larger than several of the between-checkpoint differences the field routinely reports as improvements. Under the uncorrected MDE$_{50}$, four of eleven settings are resolvable under the $256$-token protocol; under the all-pairwise Bonferroni correction ($m=45$), none are. This gap between the screening criterion and the multiplicity-corrected threshold illustrates how sharply the verdict depends on what standard of evidence is required---a dependence that is invisible when the evaluation protocol itself is unspecified.

\begin{table}[t]
\centering
\caption{Consolidated view of all experimental settings. Ranges and thresholds in percentage points; $\Delta_{\min}$ is the conservative independent bound of Eq.~\eqref{eq:delta_min} (MDE$_{50}$) computed at the $\bar{p}$ observed in each setting. Paired thresholds for the primary settings appear in Appendix~\ref{app:paired}. The Pairwise column compares the observed range to the uncorrected MDE$_{50}$ (screening criterion); the Multiplicity column applies the all-pairwise ($m=45$) Bonferroni correction. Details in \S\ref{sec:framework}.}
\label{tab:all_settings}
\centering
\resizebox{\textwidth}{!}{%
\begin{tabular}{lcrrrrrll}\toprule
\textbf{\S} & \textbf{Setting} & \textbf{Prot.} & $\bm{N}$ & \textbf{Range} & $\bm{\Delta_{\min}^{\text{(ind)}}}$ & $\bm{\Delta_{\min}^{\text{(pair)}}}$ & \textbf{Pairwise} & \textbf{Multipl.} \\\midrule
\ref{sec:full_budget} & Qwen2.5-7B LoRA GSM8K full & train. & 1319 & 3.0 & 3.8 & 2.9--3.5 & --- & --- \\
\ref{sec:budget_degradation} & Qwen2.5-7B LoRA GSM8K sub. & few-sh. & 200 & 8.0 & 9.8 & --- & Not resolv. & --- \\
\ref{sec:budget_degradation} & Qwen2.5-7B seed 42 (128~tok) & train. & 200 & 5.5 & 9.8 & 8.7--9.3 & Not resolv. & --- \\
\ref{sec:seeds} & Qwen2.5-7B seed 42 (256~tok) & train. & 200 & 12.5 & 9.7 & 8.7--9.3 & Resolvable & Not resolv. \\
\ref{sec:seeds} & Qwen2.5-7B seed 43 (256~tok) & train. & 200 & 14.0 & 9.1 & --- & Resolvable & Not resolv. \\
\ref{sec:seeds} & Qwen2.5-7B seed 44 (256~tok) & train. & 200 & 13.5 & 9.1 & --- & Resolvable & Not resolv. \\
\ref{sec:small_scale} & Qwen2.5-1.5B LoRA & zero-sh. & 200 & 3.5 & 3.4 & --- & \emph{Invalid} & --- \\
\ref{sec:full_ft} & Qwen2.5-1.5B full fine-tune & train. & 200 & 6.0 & 9.4 & --- & Not resolv. & --- \\
\ref{sec:mistral} & Mistral-7B LoRA & train. & 200 & 11.0 & 8.8 & 7.2--7.9 & \textbf{Resolvable} & Not resolv. \\
\ref{sec:mmlu} & Qwen2.5-7B MMLU (5 subd.) & --- & $\approx$250 & 6.4 & 8.8 & --- & Not resolv. & --- \\
\ref{sec:mnist} & MNIST random forest & --- & 5000 & --- & 1.4 & 1.1--1.3 & See Table~\ref{tab:mnist} & --- \\
\bottomrule
\end{tabular}}
\end{table}

% ============================================================
\section{Literature Survey: Evaluation Pipeline Reporting}\label{sec:survey}
% ============================================================

To assess whether the protocol-sensitivity problem documented in \S\ref{sec:protocol_sensitivity} extends beyond our controlled experiments, we surveyed recent (2024--2026) LLM fine-tuning papers for reporting of the generation length parameter---the specific parameter that flips the pairwise MDE$_{50}$ resolution verdict in our experiments.
...
None of the 50 surveyed papers report the maximum generation length used during evaluation. A few state that they used greedy decoding or ``default'' generation settings, but none specify the token budget. The 95\% Wilson confidence interval for this proportion is [$0\%$, $7.1\%$] (computed under simple random sampling; the convenience sample design means this should be read as a descriptive lower bound rather than a precise population estimate). This gap is consistent with the hypothesis that the evaluation pipeline---including the decoding configuration---is treated as an interchangeable constant across comparisons, when in fact a single unreported parameter can reverse the pairwise MDE$_{50}$ resolution verdict.
...
Under the $128$-token training-format evaluation (Table~\ref{tab:seed_rep}), the seed-42 trajectory has range $5.5$~pp below $\Delta_{\min}=9.8$~pp, and the rule selects soup over the empirical argmax (step~100, $52.0\%$). However, as \S\ref{sec:protocol_sensitivity} documents, this range is compressed by token truncation. At $256$~tokens the range expands to $12.5$~pp, exceeding $\Delta_{\min}$, and the rule selects argmax. The branch output therefore depends on the evaluation protocol as well as on the candidate set. We measure $\kappa$ below at the $256$-token setting to match the protocol under which the range is evaluated; the earlier $128$-token numbers are retained for comparability.
...
\begin{enumerate}
    \item \textbf{An empirical demonstration of protocol-dependent resolution.} A single decoding parameter---maximum generation length---shifts observed accuracy ranges by $2.3\times$, reverses pairwise resolution verdicts across three training seeds, and conceals evidence of overfitting. The $\Delta_{\min}$ diagnostic (MDE$_{50}$ threshold from classical power analysis) is the tool that reveals this sensitivity, establishing that evaluation resolution claims are interpretable only when the evaluation pipeline is fully specified.
    ...
    \item \textbf{A literature survey} documenting that none of 50 recent fine-tuning papers report the maximum generation length, the parameter our experiments show can determine the resolution verdict.
\end{enumerate}
...
$\Delta_{\min}$ is a pre-flight diagnostic for model selection. Before asking which model is best, ask whether the evaluation budget can tell the difference. The protocol needs only $N$ and $\bar{p}$ (defaulting to $\bar{p}=0.5$ when unknown) and returns a threshold that determines whether observed performance gaps exceed the noise floor.

The central finding is that resolution depends not only on $N$ but on the evaluation protocol itself. In controlled LoRA experiments, a single decoding parameter, the maximum generation length, shifted the observed accuracy range by $2.3\times$ (from $5.5$~pp to $12.5$~pp at $N=200$). It reversed the pairwise resolution verdict under the MDE$_{50}$ screening criterion across all three training seeds and inverted which checkpoint looked best. Between-checkpoint standard deviation more than doubled when the token limit was relaxed, because shorter generation truncates chain-of-thought reasoning and compresses across-checkpoint variance. One consequence is that a "not resolvable" verdict from $\Delta_{\min}$ can come from a pipeline that suppresses signal rather than from genuinely indistinguishable candidates.

The practical implication is straightforward. If the evaluation pipeline is incompletely specified, the verdict of any resolution diagnostic is unverifiable. A survey of 50 recent fine-tuning papers suggests that this incompleteness is the norm. None of the papers surveyed report the maximum generation length that our experiments show can determine the verdict. The problem of underpowered evaluation is compounded by a problem of unaccountable evaluation. We recommend that $\Delta_{\min}$ computation and full pipeline specification become standard submission requirements for any benchmark evaluation. The quick-reference table, power-curve calibration, and diagnostic pitfall catalog are intended as actionable resources for practitioners and reviewers alike.

% ============================================================
\appendix
% ============================================================

\section{Multiple Comparison Correction}\label{app:proof}

The Bonferroni correction for $m$ comparisons sets the per-comparison significance level to $\alpha' = \alpha / m$. For the two-proportion $z$-test, the minimum detectable effect at level $\alpha'$ is:
\[
...
\]
This correction is conservative when candidates are positively correlated, as adjacent checkpoints are. Holm-Bonferroni step-down correction and Benjamini-Hochberg false discovery rate control would produce tighter thresholds; we report Bonferroni as a strict upper bound throughout. For $m=45$ pairwise comparisons among $10$ checkpoints, $\alpha' = 0.05/45$, $z_{0.05/90} \approx 3.26$, and $\Delta_{\min}^{(45)} \approx 3.26 \cdot 0.05 = 0.162 = 16.2$~pp at $\bar{p}=0.5$.
...
Table~\ref{tab:paired_vs_indep} shows the comparison for the settings with available per-sample predictions.

\begin{table}[h]
\centering
\caption{Paired vs.\ independent $\Delta_{\min}$ thresholds (pp) for the experimental settings with available per-sample predictions.}
...
The paired thresholds are $3$--$24\%$ tighter than the independent bound, with the largest gaps occurring at larger $N$ (Qwen full test set, MNIST) where the lower noise floor amplifies the relative benefit of pairing. For the subsampled experiments at $N=200$, the reduction is $5$--$11\%$. The reduction does not change the pairwise MDE$_{50}$ screening verdicts reported in Table~\ref{tab:all_settings}. The independent bound is a safe pre-experimental diagnostic that does not overstate the resolution problem.
...
\begin{table}[h]
\centering
\caption{Adjacent two-proportion $z$-tests (few-shot protocol, $N=200$).}
\label{tab:adjacent}
\begin{tabular}{lccc}\toprule
...
\begin{table}[h]
\centering
\caption{GSM8K accuracy (training-format prompt) across three seeds at $128$~tokens (left) and $256$~tokens (right).}
\label{tab:seed_rep}
\begin{tabular}{lcccccc}\toprule
\multirow{2}{*}{\textbf{Step}} & \multicolumn{3}{c}{\textbf{128~tokens}} & \multicolumn{3}{c}{\textbf{256~tokens}} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
 & \textbf{Seed 42} & \textbf{Seed 43} & \textbf{Seed 44} & \textbf{Seed 42} & \textbf{Seed 43} & \textbf{Seed 44} \\\midrule
...
500 & $49.5\%$ & $52.0\%$ & $51.5\%$ & $57.5\%$ & $67.0\%$ & $66.5\%$ \\\midrule
Range & $5.5$~pp & $5.0$~pp & $3.0$~pp & $12.5$~pp & $14.0$~pp & $13.5$~pp \\\bottomrule
\end{tabular}
\end{table}
