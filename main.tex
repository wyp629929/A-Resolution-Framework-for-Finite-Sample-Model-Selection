...
\begin{enumerate}
    ...
    \item \textbf{Empirical validation across budgets and domains.} We validate the framework through controlled LoRA fine-tuning experiments (two model families: Qwen2.5-7B and Mistral-7B, 1.5B scale, full fine-tuning, multiple random seeds), tracing how resolution degrades under subsampling and showing that a single decoding parameter (maximum generation length) can shift observed ranges by $2.3\times$ and reverse resolvability verdicts. The pattern is further replicated on classical ML model selection (random forests on MNIST) and synthetic simulations.
...
Two additional random seeds (43,~44) were trained. Under the $128$-token training-format evaluation (Table~\ref{tab:seed_rep}), seed~43 has range $5.0$~pp and seed~44 has range $3.0$~pp, both below $\Delta_{\min}=9.8$~pp. However, as Section~\ref{sec:adaptive} documents, these ranges are compressed by token truncation. At $256$~tokens, all three seeds become resolvable: seed~43 has range $14.0$~pp and seed~44 has range $13.5$~pp, both exceeding $\Delta_{\min}$. Critically, the $256$-token data reveal a pattern that the $128$-token evaluation entirely masked: seeds~43 and~44 show negative trends (Spearman $\rho=-0.77$, $p=0.009$ and $\rho=-0.63$, $p=0.050$ respectively; Holm-Bonferroni-corrected threshold $\alpha=0.0050$), with the earliest checkpoint (step~50) outperforming the last by $12.5$~pp and $10.5$~pp. This is evidence suggestive of overfitting after an initial learning phase, though we caution that these correlations are estimated from $n=10$ checkpoints per seed and are driven in part by the high accuracy at step~50. The pattern is invisible under a generation limit that truncates the model's output. Seed~42 does not share this trend ($\rho=+0.29$, $p=0.43$), confirming that protocol sensitivity and overfitting susceptibility are trajectory-specific.
...
Table~\ref{tab:all_settings} collects every experimental setting reported in \S\ref{sec:primary} and \S\ref{sec:robustness} with its evaluation protocol, budget, observed range, threshold and verdict. The same Qwen2.5-7B checkpoints appear five times at $N=200$ under different protocols: the few-shot evaluation (range $8.0$~pp), the $128$-token training-format evaluation ($5.5$~pp), and three $256$-token training-format evaluations showing all three seeds resolvable ($12.5$, $14.0$, $13.5$~pp). The $2.5$--$9.0$~pp discrepancy between protocols---driven by a single decoding parameter (\S\ref{sec:protocol_sensitivity})---is itself larger than several of the between-checkpoint differences the field routinely reports as improvements. Under the $256$-token protocol, four of eleven settings are resolvable; under the uncorrected $128$-token protocol, only Mistral registers as resolvable, confirming that the truncation artefact systematically suppresses signal.

\begin{table}[t]
\centering
\caption{Consolidated view of all experimental settings. Ranges and thresholds in percentage points; $\Delta_{\min}$ is the conservative independent bound of Eq.~\eqref{eq:delta_min} (MDE$_{50}$) computed at the $\bar{p}$ observed in each setting. Paired thresholds for the primary settings appear in Appendix~\ref{app:paired}. The Pairwise column uses the $m=10$ Holm-Bonferroni-corrected threshold for the best-vs-worst comparison; seeds 43 and 44 remain resolvable under this correction, seed 42 does not. The Multiplicity column uses the all-pairwise threshold ($m=45$); details in \S\ref{sec:framework}.}
...
\ref{sec:seeds} & Qwen2.5-7B LoRA, seed 43 (256~tok) & training & 200 & 14.0 & 9.1 & --- & Resolvable & Resolvable \\
...
\ref{sec:mistral} & Mistral-7B LoRA & training & 200 & 11.0 & 8.8 & 7.2--7.9 & \textbf{Resolvable} & \textbf{Resolvable} \\
...
The $256$-token data also reveal a pattern that the $128$-token evaluation entirely concealed: seeds~43 and~44 show significant negative trends (Spearman $\rho=-0.77$, $p=0.009$ and $\rho=-0.63$, $p=0.050$), with the earliest checkpoint outperforming the final one---evidence of overfitting that is invisible under truncated generation. Seed~42 does not share this trend ($\rho=+0.29$, $p=0.43$). All $256$-token checkpoints appear in Appendix~\ref{tab:seed_rep}.

The $\Delta_{\min}$ comparisons in this section use the $256$-token evaluation for Qwen and the $256$-token evaluation for Mistral; the earlier $128$-token results (Tables~\ref{tab:trajectory},~\ref{tab:seed_rep}) are retained for comparability with published work.
...
\begin{enumerate}
    ...
    \item \textbf{A literature survey} documenting that none of 50 recent fine-tuning papers report the maximum generation length, the parameter our experiments show can reverse the resolvability verdict.
\end{enumerate}
...
The literature survey summary data are available in the project repository (see Code and data availability).
