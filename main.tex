...
\abstract{Model selection in data science and machine learning is routinely performed on finite validation sets, often at budgets where classical statistics predicts that rankings cannot be resolved. This paper proposes a pre-experimental diagnostic protocol that determines whether an evaluation budget can support statistically distinguishable comparisons before that budget is spent. The protocol centers on $\Delta_{\min}$, a resolution threshold derived from the two-proportion $z$-test, and embeds a decision rule (Decision Rule~1) that specifies the condition under which uniform model averaging (soup) provably dominates argmax selection. We validate the protocol on Qwen2.5-7B and Mistral-7B across multiple seeds, scales and training paradigms, and replicate the pattern on classical supervised learning (random forests on MNIST) and controlled simulation. At full GSM8K ($N=1319$), $\Delta_{\min}=3.8$~pp exceeds the observed checkpoint range of $3.0$~pp. Crucially, the protocol does not uniformly predict failure: for Mistral-7B at $N=200$, $\Delta_{\min}=8.11$~pp is exceeded by the $10.5$~pp observed range. Bootstrap analysis, Monte Carlo power calibration, an extension to continuous metrics via the Delta method, and an explicit characterization of the regimes in which the diagnostic itself breaks down accompany the framework, together with a practitioner quick-reference table and a catalog of nine pitfalls. An exploratory convenience-sample survey of 36 papers (78 claims) finds that $71\%$ of claimed improvements fall below $\Delta_{\min}$ at their reported evaluation budgets.}
...
\]
\noindent (Proof in Appendix~\ref{app:rule_proof}; Figure~\ref{fig:decision_rule}.)
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\textwidth]{figures/fig_decision_rule_contour.png}
\caption{Decision Rule~1 contour plot ($m=10$, $N=200$, $\bar p=0.50$). The colour map shows $\mathbb{E}[\ell(\mathrm{argmax})] - \mathbb{E}[\ell(\mathrm{soup})] = \kappa - g(\delta)$. The black curve is the break-even $\kappa = g(\delta)$ (uniform spacing); the orange dashed curve is the bang-bang envelope; the green dashed line is the sufficient condition $\kappa \geq (m-1)\Delta_{\min}/m$. The gray dotted line marks $\delta = \Delta_{\min}$ with $\mathcal{P}_\Delta$ shaded; the red X marks the Qwen2.5-7B seed~42 empirical point ($\delta\approx 12.5$~pp, $\kappa\approx -4.5$~pp). Top-axis $\delta/\mathrm{SE}_1$ uses $\bar p=0.50$ (synthetic surface) and does not apply to the empirical point.}
\label{fig:decision_rule}
\end{figure}
...
Two additional random seeds (43,~44) confirm the pattern. Seed~43 has range $5.0$~pp and seed~44 has range $3.0$~pp, both below $\Delta_{\min}=9.8$~pp (seed~42, the primary trajectory, has range $5.5$~pp). Spearman trends are non-significant across all three seeds; no adjacent pair reaches significance.

\subsection{Smaller Model Scale: A Case Where the Diagnostic Fails}\label{sec:small_scale}

Qwen2.5-1.5B with the same LoRA configuration (zero-shot prompting) yields checkpoint accuracies between $1.5\%$ and $5.0\%$, an observed range of $3.5$~pp against $\Delta_{\min}=3.4$~pp. Read mechanically, the rule would classify this comparison as (just) resolvable, with a two-proportion $z$-test giving $p\approx0.049$ for the best-versus-worst pair.
...
Full fine-tuning (no LoRA) at 1.5B scale (training-format prompt) gives range $6.0$~pp, $\Delta_{\min}=9.4$~pp, also below the resolution threshold.
...
What the comparison does not establish is that $\Delta_{\min}$ separates converged trajectories that differ only in candidate quality, since we have one converged trajectory family and one unconverged one. The clean version of this experiment trains both families to convergence and varies only the data budget between candidate pairs. Two additional training seeds (43,~44) underscore this concern: seed~43 has a $5.5$~pp range against $\Delta_{\min}=8.39$~pp, and seed~44 has an $8.0$~pp range against $\Delta_{\min}=8.67$~pp. Both fall below the resolution threshold, in contrast to the primary seed. The additional seeds are consistent with the view that the primary seed's resolvable range reflects training progress rather than stationary checkpoint variation. We flag both confounds in \S\ref{sec:limitations}.
...
\ref{sec:seeds} & Qwen2.5-7B LoRA, seed 42 & training & 200 & 5.5 & 9.8 & Not resolvable \\
\ref{sec:seeds} & Qwen2.5-7B LoRA, seed 43 & training & 200 & 5.0 & 9.8 & Not resolvable \\
\ref{sec:seeds} & Qwen2.5-7B LoRA, seed 44 & training & 200 & 3.0 & 9.8 & Not resolvable \\
...
On the seed-42 trajectory under the evaluation protocol used throughout \S\ref{sec:primary} ($\Delta_{\min}=9.8$~pp, range $5.5$~pp) the rule selects soup over the empirical argmax (step~100, $52.0\%$). Across all three training seeds the observed ranges are $5.5$, $5.0$ and $3.0$~pp, all below $\Delta_{\min}$, so the rule selects soup in every case; for Mistral-7B (range $10.5$~pp, $\Delta_{\min}=8.11$~pp) it selects argmax. The rule's branch output is stable within an architecture across seeds and switches across architectures, which is the behaviour a useful diagnostic should exhibit: neither constant nor seed-dependent.

To measure $\kappa$ directly we constructed a uniform weight average of the ten Qwen2.5-7B LoRA adapters (seed~42; see repository for the averaging and evaluation protocol). The individual checkpoint accuracies under this evaluation range from $49.5\%$ to $62.0\%$ (mean $56.0\%$). The averaged model scores $51.5\%$, giving $\kappa \approx -4.5$~pp. The sufficient condition $\kappa \geq \frac{m-1}{m}\Delta_{\min} \approx 8.8$~pp is therefore not met. This need not conflict with the broader soup literature, which averages full fine-tuned weights rather than LoRA adapters and operates on converged checkpoints separated by hyperparameter variation rather than early-stopped steps from a single trajectory; LoRA averaging may be effective in different regimes. For the present setting the rule selects argmax (the observed range of $12.5$~pp exceeds $\Delta_{\min}=9.8$~pp), and argmax at $62.0\%$ indeed outperforms soup at $51.5\%$, so the branch selection and the regret ordering agree.
...
\begin{enumerate}
    \item \textbf{A discriminative empirical validation} showing that $\Delta_{\min}$ correctly distinguishes resolvable (Mistral-7B) from non-resolvable (Qwen2.5-7B) configurations rather than uniformly flagging noise.
    \item \textbf{A decision rule with a proved sufficient condition} (Decision Rule~1), which identifies exactly when uniform averaging dominates argmax selection in expected regret, together with an explicit statement of which half of that condition our experiments verify and which they do not.
...
\emph{The soup gain is measured for one trajectory and is negative.} For the Qwen2.5-7B seed~42 checkpoints at $N=200$, the uniform LoRA weight average gives $\kappa \approx -4.5$~pp, so the sufficient condition in Decision Rule~1 is not met, and the rule relies on its branch-selection heuristic rather than on the proved regret bound (\S\ref{sec:adaptive}). Whether $\kappa$ is systematically negative for LoRA adapters from a shared trajectory, or depends on the number of checkpoints and their separation in training time, remains open. The checkpoints used here are saved at $50$-step intervals; averaging adapters saved at closer intervals may yield different results.
...
The most instructive finding is the divergence between Qwen2.5-7B and Mistral-7B at the same budget ($N=200$): $\Delta_{\min}=9.8$~pp exceeds the Qwen range of $8.0$~pp under few-shot prompting ($5.5$~pp under training-format prompting), while $\Delta_{\min}=8.11$~pp falls below the Mistral range of $10.5$~pp. The threshold moves as well as the range, because Mistral's lower mean accuracy tightens it; a diagnostic insensitive to $\bar{p}$ would have classified both identically. This discriminative behaviour is the strongest evidence that $\Delta_{\min}$ tracks genuine resolution constraints rather than uniformly classifying comparisons as noise, subject to the confound we identify in \S\ref{sec:mistral}. Bootstrap analysis, power-curve calibration and paired-versus-independent comparisons (Appendix~\ref{app:paired}) support the threshold's interpretation, and \S\ref{sec:small_scale} documents a regime in which it should not be applied at all. An exploratory survey of 36 recent papers (78 claims) finds that 71\% of reported improvements fall below the resolution threshold implied by their budgets; the problem is not confined to our own experiments.
